        ...
        {paidCases.map((c) => {
          ...
          const card = (
            <GlassCard className={`flex flex-col p-0 relative overflow-hidden h-full ${!valid ? "opacity-60" : ""}`}>
              ...
              {!valid && (
                <div className="absolute top-3 right-3 text-[10px] uppercase tracking-wider font-bold text-red-300/90 bg-red-950/60 border border-red-400/30 px-2 py-1 rounded max-w-[160px] text-center leading-tight">
                  {INVALID_CASE_MSG}
                </div>
              )}
              {valid && !canAfford && (
          ...
          return valid ? (
            ...
            <div key={c.id} className="group block rounded-2xl cursor-not-allowed">
              {card}
            </div>
          );
        })}
