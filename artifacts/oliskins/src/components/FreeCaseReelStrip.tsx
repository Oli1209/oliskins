 ...
 * Free-case reel.
 *
 * Independent copy of the regular reel, tuned to eliminate visible tile
 * "jumping" during the roll:
 *  - tiles have fixed width/height (no aspect-ratio reflow)
 *  - image container is fixed size; `object-contain` prevents image-driven
 *    layout shifts
 *  - reel sequence is built ONCE per roll and frozen in a ref so React
 *    re-renders cannot reshuffle items mid-animation
 *  - track is a left-aligned flex row with `flex-wrap: nowrap` and a fixed
 *    gap (no `justify-*`)
 */
export function FreeCaseReelStrip({
  caseData,
  winningItem,
  onResolved,
  ...
  // Build the reel sequence once and freeze it in a ref so subsequent
  // re-renders (e.g. cooldown ticks bubbling through context) cannot
  // cause `pickRandomDrop` to re-run mid-animation.
  ...
  const frozenItemsRef = useRef(initialReelItems);
  const reelItems = frozenItemsRef.current;
